...
/** `alert_loop.ALERTING_FAMILIES`. GPUs only. Consoles alerted for six days
 *  (2026-09-08 to 09-14) and were withdrawn at the owner's request; they are
 *  still tracked for comps. Keep this default in step with the tracker's. */
export const ALERTING_FAMILIES = new Set(
  (process.env.ALERTING_FAMILIES ?? "gpu")
    .split(",")
    .map((f) => f.trim())
    .filter(Boolean),
);
